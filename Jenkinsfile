    ...
    stages {
        stage('SCM Checkout') {
            steps {
                ...
                git 'https://github.com/lavudishashikala/Banking.git'
       ...
       stage("Docker build"){
            steps {
				sh 'docker version'
				sh "docker build -t swathi683/bankapp-eta-app:${BUILD_NUMBER} ."
				sh 'docker image list'
				sh "docker tag swathi683/bankapp-eta-app:${BUILD_NUMBER} swathi683/bankapp-eta-app:latest"
            }
        }
		...
		stage('Push2DockerHub') {
			...
			steps {
				sh "docker push swathi683/bankapp-eta-app:latest"
			}
		}
